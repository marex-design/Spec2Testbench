...
def _builds(case_id: str):
    spec = Specification.from_yaml(ROOT / "benchmark" / "analogcoder_pro" / "specs" / f"{case_id}.yaml")
...
def test_transient_harness_does_not_modify_bias_source():
    build = _build_by_key("p28_schmitt", "schmitt")
...
def test_signal_dc_override_requires_authority():
    spec = Specification(
        ...
        circuit_type=Specification.from_yaml(ROOT / "benchmark" / "analogcoder_pro" / "specs" / "p04_amplifier.yaml").circuit_type,
...
def test_analysis_specific_decks_are_separate():
    ...
    assert {build.analysis_key for build in builds} == {"ac_gain"}
