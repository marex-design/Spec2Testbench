...
P01_SPEC = Path("benchmark/analogcoder_pro/specs/p01_amplifier.yaml")
P01_NETLIST = Path("benchmark/analogcoder_pro/p01_amplifier.cir")
P05_SPEC = Path("benchmark/analogcoder_pro/specs/p05_amplifier.yaml")
...
P17_SPEC = Path("benchmark/analogcoder_pro/specs/p17_currentmirror.yaml")
...
def _targeted_spec(spec_path: Path, *, case_id: str, metric_name: str) -> Specification:
    ...
    specification = specification.project_verification_metrics([metric_name])
    specification.case_id = case_id
    specification.parent_circuit_id = spec_path.stem
...
def test_stub_provider_does_not_read_deepseek_key(monkeypatch):
    specification = _targeted_spec(P10_SPEC, case_id="stub_no_key", metric_name="lowpass_attenuation_db")
    deterministic_tb = FrameworkTestBenchGenerator(use_llm=False).generate(
        specification,
        netlist_path=P10_NETLIST,
    )
...
def test_stub_plan_preserves_requested_metrics():
    ...
    assert [measurement.metric_name for measurement in outcome.parsed_plan.measurements] == specification.verification_metric_names()
