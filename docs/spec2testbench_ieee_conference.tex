...
\author{
\IEEEauthorblockN{Exaucé Maruba Kambale}
\IEEEauthorblockA{
...
\and
\IEEEauthorblockN{Christian Moanda Ndeko}
\IEEEauthorblockA{
Faculty of Polytechnic\\
University of Kinshasa\\
Kinshasa, DR Congo\\
christianmoanda@yahoo.fr
}
\and
\IEEEauthorblockN{Kyandoghere Kyamakya}
\IEEEauthorblockA{
...
\subsection{Reproducible CLI Usage}
The framework is exposed through a command-line interface, which is useful both for beginners and for reproducible experimental campaigns. The main commands used in the present work are:

\begin{verbatim}
spec2testbench verify --specs <spec.yaml> --netlist <circuit.cir> --no-llm
spec2testbench generate --specs <spec.yaml> --output <testbench.py> --no-llm
spec2testbench draw --netlist <circuit.cir> --output <schematic.png>
spec2testbench providers
\end{verbatim}

For paper-facing benchmark runs, the most important command is the deterministic verification mode:

\begin{verbatim}
spec2testbench verify --specs examples/reference_28_specs/<case>.yaml \
  --netlist benchmark_reference_28/<case>.cir --no-llm
\end{verbatim}

This explicit CLI layer is important because it makes the framework auditable by readers: the same commands can be reused to regenerate testbenches, execute ngspice, export schematics, and reproduce the benchmark-aligned verdict tables reported in this paper.
...
The strongest current evidence comes from two non-LLM campaigns that answer different questions. The first is a broad 35-circuit execution campaign focused on end-to-end operational coverage. The second is a benchmark-aligned 28-topology campaign in which each circuit is paired with an explicit YAML specification and scored through the proposed verdict semantics. Table~\ref{tab:campaign} summarizes the broad execution campaign, while Table~\ref{tab:reference28} summarizes the stricter benchmark-aligned pass/fail evidence.
...
\begin{table}[t]
\caption{Aggregated results of the benchmark-aligned 28-topology verification campaign}
\label{tab:reference28}
\centering
\begin{tabular}{lc}
\toprule
Metric & Value \\
\midrule
Reference topologies & 28 \\
PASS verdicts & 28/28 \\
RUN verdicts & 0/28 \\
FAIL verdicts & 0/28 \\
Mean compliance score & 1.000 \\
Topology-level pass rate & 100\% \\
\bottomrule
\end{tabular}
\end{table}

This second result is particularly important for the paper because it is stricter than merely ``35/35 runs completed.'' Each of the 28 reference circuits is associated with benchmark-aligned metrics, generated specifications, extracted measurements, and a final compliance verdict. After correcting the previously weak cases through better nominal DC operating-point handling, better band-pass metric targeting, and cleaner oscillator stimulus/output handling, the audited deterministic flow now reaches 28/28 PASS. This makes the evaluation substantially stronger: the framework is not only runnable, but also specification-compliant on the curated benchmark-aligned set used for paper reporting.
...
The topology-level summary also helps interpret compliance. Earlier weak cases were useful because they revealed that simulator completion is not enough: node targeting and excitation choice can still produce measurements that are technically extractable but verification-wise unconvincing. In the final audited campaign, those cases were resolved by targeted methodological fixes rather than by hiding them inside a broader benchmark. Specifically, the framework was improved to measure nominal DC operating points correctly, to use a more appropriate bandwidth definition for band-pass filters, and to avoid conflicting injected stimuli for self-oscillating circuits. The fact that these fixes converted residual weak cases into final PASS outcomes strengthens the claim that the framework is becoming methodologically better, not merely numerically luckier.
