...
class AnalysisConfig:
    ...
    def to_spice(self) -> str:
        ...
        if self.type == AnalysisType.DC:
            ...
            stop = self.parameters.get('stop', 5)
            step = self.parameters.get('step', 0.01)
            force_sweep = bool(self.parameters.get('force_sweep', False))
            if not force_sweep and abs(float(stop) - float(start)) <= max(abs(float(step)), 1e-18):
                return ".OP"
...
class TestBench:
    ...
    """Identifiant unique de variante propagé jusqu'au simulateur"""
    ...
    """Métadonnées d'exécution et de traçabilité"""
