...
class Specification:
    ...
    def input_nodes(self) -> List[str]:
        # Schema v2 makes port roles authoritative.  Falling back to legacy
        # input_conditions can misclassify reference/supply nodes as user
        # excitation ports (e.g. current mirrors).
    ...
    def verification_metric_names(self) -> List[str]:
        """Return metrics that are allowed to influence the verification verdict.

        Legacy v1 specifications treat every performance target as required.
        Strict v2 specifications instead derive the required runtime contract from
        mandatory functional requirements explicitly marked ``executable``.
        Diagnostic-only targets therefore remain observable metadata and can never
        create a false FAIL/PASS in the scientific contract.
        """
        if str(self.schema_version) == "2.0" and self.functional_requirements:
            names: List[str] = []
            for requirement in self.functional_requirements:
                if not isinstance(requirement, dict):
                    continue
                if not bool(requirement.get("mandatory", True)):
                    continue
                if str(requirement.get("implementation_status", "")) != "executable":
                    continue
                metric = requirement.get("executable_metric")
                if metric and metric in self.performance_targets and metric not in names:
                    names.append(str(metric))
            return names
        return list(self.performance_targets.keys())

    def diagnostic_metric_names(self) -> List[str]:
        """Return explicitly diagnostic targets that do not affect the v2 verdict."""
        required = set(self.verification_metric_names())
        return [name for name in self.performance_targets if name not in required]

    def verification_performance_targets(self) -> Dict[str, Any]:
        """Return the threshold mapping used by deterministic verdict logic."""
        return {name: self.performance_targets[name] for name in self.verification_metric_names()}

    def project_verification_metrics(self, metric_names: List[str]) -> "Specification":
        """Create an isolated runtime projection for a subset of verdict metrics.

        The canonical YAML remains unchanged. This is useful for experiments that
        compare planners on one metric/analysis while preserving one v2 source of
        truth instead of maintaining duplicate YAML dialects.
        """
        requested = [name for name in metric_names if name in self.performance_targets]
        clone = copy.deepcopy(self)
        clone.performance_targets = {name: copy.deepcopy(self.performance_targets[name]) for name in requested}
        if str(clone.schema_version) == "2.0":
            clone.functional_requirements = [
                copy.deepcopy(requirement)
                for requirement in self.functional_requirements
                if isinstance(requirement, dict)
                and requirement.get("implementation_status") == "executable"
                and requirement.get("executable_metric") in requested
            ]
        return clone
    ...
    def from_yaml(cls, path: Path) -> "Specification":
        ...
        return cls(
            ...
            provenance=data.get("provenance", data.get("source", {})) if isinstance(data.get("provenance", data.get("source", {})), dict) else {},
            stimuli=data.get("stimuli", []) if isinstance(data.get("stimuli", []), list) else [],
            analyses=data.get("analyses", []) if isinstance(data.get("analyses", []), list) else [],
            functional_requirements=data.get("functional_requirements", []) if isinstance(data.get("functional_requirements", []), list) else [],
        )
