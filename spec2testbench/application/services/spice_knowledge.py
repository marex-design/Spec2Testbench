...
def create_knowledge_manifests(
    ...
    *,
    ...
    source_acp28 = yaml.safe_load((ROOT / "experiments/hybrid_feedback/acp28_manifest.yaml").read_text(encoding="utf-8"))
    source_cases = list(source_acp28.get("cases", []))
    smoke_payload = {
        **source_acp28,
        "version": f"{knowledge_version}_acp28_smoke",
        "case_count": min(7, len(source_cases)),
        "cases": source_cases[:7],
        "notes": [
            "Derived from the current ACP-28 hybrid manifest.",
            f"Used for {knowledge_version} retrieval/stub smoke validation only.",
        ],
    }
    frozen_payload = {
        **source_acp28,
        "version": f"{knowledge_version}_acp28_full",
        "notes": [
            "Derived from the current ACP-28 hybrid manifest.",
            f"Used for {knowledge_version} deterministic parity and stub replay.",
        ],
    }
