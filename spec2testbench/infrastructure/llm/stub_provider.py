...
class DeterministicStubProvider(LLMProvider):
    ...
    def _build_plan(self, payload: dict[str, Any]) -> TestbenchPlan:
        ...
        role_nodes = payload.get("role_nodes")

        input_node = self._choose_input_node(available_nodes, deterministic_summary, role_nodes)
        output_node = self._choose_output_node(available_nodes, deterministic_summary, role_nodes)
    ...
    @staticmethod
    def _choose_input_node(
        available_nodes: list[str],
        summary: dict[str, Any],
        role_nodes: dict[str, Any] | None = None,
    ) -> str | None:
        if role_nodes is not None:
            declared = [str(node) for node in (role_nodes.get("input") or []) if node]
            if declared:
                return declared[0]
            # Presence of role_nodes means schema-v2 roles are authoritative.
            # A reference-only/current-bias circuit therefore has no input
            # excitation to invent.
            return None
    ...
    @staticmethod
    def _choose_output_node(
        available_nodes: list[str],
        summary: dict[str, Any],
        role_nodes: dict[str, Any] | None = None,
    ) -> str | None:
        if role_nodes is not None:
            declared = [str(node) for node in (role_nodes.get("output") or []) if node]
            if declared:
                return declared[0]
