...
class TestBenchGenerator(ITestBenchGenerator):
    ...
    def generate(self, specification: Specification, netlist_path: Optional[Path] = None) -> TestBench:
        ...
        if getattr(specification, "is_v2", False):
            return self._generate_v2_contract(specification, netlist_path=netlist_path)
    ...
    def _generate_v2_contract(self, specification: Specification, netlist_path: Optional[Path] = None) -> TestBench:
        """Compile a strict v2 verification contract deterministically."""
        stimuli: List[Stimulus] = []
        for item in specification.stimuli:
            kind = str(item.get("kind", "DC")).lower()
            params = dict(item.get("parameters") or {})
            if "dc_value" in params and "value" not in params:
                params["value"] = params["dc_value"]
            stimuli.append(Stimulus(
                name=str(item.get("source") or item.get("id") or f"stim_{len(stimuli)}"),
                type=kind, parameters=params,
                node_positive=str(item.get("node_positive") or (specification.ports.get("input") or ["1"])[0]),
                node_negative=str(item.get("node_negative") or "0"),
            ))
        analyses: List[AnalysisConfig] = []
        type_map = {"OP": AnalysisType.OP, "DC": AnalysisType.DC, "AC": AnalysisType.AC,
                    "TRAN": AnalysisType.TRANSIENT, "TRANSIENT": AnalysisType.TRANSIENT,
                    "FOURIER": AnalysisType.FOURIER}
        for item in specification.analyses:
            atype = type_map.get(str(item.get("type", "DC")).upper(), AnalysisType.DC)
            params = dict(item.get("parameters") or {})
            aliases = {"frequency_start_hz":"start_freq", "frequency_stop_hz":"stop_freq",
                       "stop_time_s":"end_time", "time_step_s":"step_time", "start_time_s":"start_time",
                       "dc_source":"source", "dc_start":"start", "dc_stop":"stop", "dc_step":"step"}
            for src,dst in aliases.items():
                if src in params and dst not in params: params[dst]=params[src]
            analyses.append(AnalysisConfig(type=atype, parameters=params))
        required_metrics = specification.verification_metric_names()
        measurements: List[Measurement] = []
        for metric in required_metrics:
            target = specification.performance_targets.get(metric, {}) or {}
            expression = "min(abs(Id)) from auxiliary ngspice OP-bias probe" if metric == "minimum_device_drain_current_a" else metric
            measurements.append(Measurement(name=metric, expression=expression, expected_min=target.get("min"),
                expected_max=target.get("max"), unit=str(target.get("unit", "")),
                node=((specification.ports.get("output") or [None])[0])))
        tb = TestBench(name=f"{specification.case_id or specification.name}_deterministic", category="strict_v2",
            circuit_name=specification.name, netlist_path=str(netlist_path) if netlist_path else None,
            stimuli=stimuli, analyses=analyses, measurements=measurements, temperature=specification.nominal_temperature,
            metadata={"case_id":specification.case_id, "schema_version":"2.0", "strict_contract":True,
                "required_metrics":required_metrics, "mandatory_requirement_count":len(specification.mandatory_requirements()),
                "input_node":((specification.ports.get("input") or [None])[0]),
                "output_node":((specification.ports.get("output") or [None])[0]),
                "measurement_context":{"input_node":((specification.ports.get("input") or [None])[0]),
                                       "output_node":((specification.ports.get("output") or [None])[0])},
                "needs_op_bias_probe":"minimum_device_drain_current_a" in required_metrics})
        tb.generate_pyspice_code()
        return tb
