...
class SpecChecker(ISpecChecker):
    ...
    def verify(self, 
        ...
        """
        logger.info(f"Verifying {len(specification.verification_metric_names())} verdict metrics")
        ...
        # Verify each metric
        results = []
        for metric_name, expected in specification.verification_performance_targets().items():
    ...
    def generate_assertions(self, specification: Specification) -> str:
        ...
        for metric_name, target in specification.verification_performance_targets().items():
            expected_min = target.get("min") if isinstance(target, dict) else target
            expected_max = target.get("max") if isinstance(target, dict) else None
            unit = target.get("unit", "") if isinstance(target, dict) else ""
